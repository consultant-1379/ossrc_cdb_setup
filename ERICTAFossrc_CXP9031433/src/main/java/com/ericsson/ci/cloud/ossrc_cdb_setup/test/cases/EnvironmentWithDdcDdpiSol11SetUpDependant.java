...
public class EnvironmentWithDdcDdpiSol11SetUpDependant extends TorTestCaseHelper implements
        TestCase {
    ...
    @Context(context = { Context.CLI })
    @Test(dependsOnMethods = "verifyEnvironmentInstallation",retryAnalyzer = TestRetry.class)
    public void verifyEnvironmentInstallWithDdcDdpiSol11() {
        assertTrue(getOperator().executeDdcDdpiSol11Setup());
    }

    @TestId(id = "CIP-8129_Func_2")
    @Context(context = { Context.CLI })
    @Test(dependsOnMethods = "verifyEnvironmentInstallWithDdcDdpiSol11")
