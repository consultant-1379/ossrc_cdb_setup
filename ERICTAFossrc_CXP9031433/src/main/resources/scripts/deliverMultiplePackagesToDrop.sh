...
usage ()
{
  echo "Usage: $cmd [-D for Debug] [-a GAVs_success] [-P product][-i intendedDrop] [-p filePath] [-f fileName] [-t packageType] [-u user] [-e email] [-o Task options either 1,2 or 3 ] " 1>&2
...
[ $# -gt 0 ] || usage
while getopts ":a:P:i:p:f:t:u:e:o:ncdD" opt; do
  case $opt in
    a)
      GAVs_success=$OPTARG
      ;;
